...
class ExperimentStorage(AbstractContextManager):
    ...
    def __init__(
        ...
        try:
            with docker_client_context(
                ...
                self._container = client.containers.run(
                    ...
                    volumes={f"{exp_path}": {"bind": "/opt/expeca", "mode": "rw"}},
                )
        ...
        try:
            ...
            with ThreadPoolExecutor() as tpool:
                ...
                def _add_storage(host: AinurHost) -> Tuple[AinurHost, Volume]:
                    logger.info(
                    ...
                    try:
                        with docker_client_context(
                            base_url=f"{host.management_ip.ip}:" f"{daemon_port}"
                        ) as client:
                        ...
                        logger.critical(
                            "Exception when adding Docker volume on " f"host {host}."
                        )
                        logger.exception(e)
                        with ex_cond:
                            exceptions.append(e)
    ...
    def tear_down(
        self,
        default_wait_seconds: float = 0.01,
        max_wait_seconds: float = 5.0,
        force_threshold_seconds: float = 5 * 60.0,
    ) -> None:
        ...
        with ThreadPoolExecutor() as tpool:
            ...
            def _remove_vol(host_vol: Tuple[AinurHost, Volume]):
                ...
                ti = time.monotonic()
                while True:
                    if (time.monotonic() - ti) < force_threshold_seconds:
                        # if we haven't reached the force threshold, try nicely
                        try:
                            vol.remove()
                            logger.info(
                                f"Successfully removed Docker volume "
                            ...
                            if e.status_code == 409:
                                logger.warning(
                                    f"Volume {vol.name} on host {host} "
                                    f"is busy, waiting to remove."
                                )
                                # duplicate waiting time on each try
                                time.sleep(wait)
                                wait = max(wait * 2, max_wait_seconds)
                            else:
                                logger.exception(e)
                                with exc_cond:
                                    exceptions.append(e)
                        ...
                        try:
                            logger.warning(
                                f"Forcing removal of Docker volume "
                                f"{vol.name} from host {host}."
                            )
                            ...
                            logger.error(
                                f"Could not remove Docker volume {vol.name} "
                                f"from host {host}."
                            )
                            logger.exception(e)
                            with exc_cond:
