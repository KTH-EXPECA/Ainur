...
class RadioHostManager(AbstractContextManager):
    ...
    def __init__(self,
        ...
        # 1. setup the ips first !!SETUP IPS FIRST!!
        # build an Ansible inventory from the hosts
        self._radiohosts_inventory = {
            'all': {
                'hosts': {name: self._configs[name].to_dict() for name in
                          self._configs}
            }
        }
        ...
        # build a collection of (future) configured radiohosts
        self._con_radiohosts = frozendict({
            name: ConfiguredRadioHost(
                ansible_host=,
                management_ip=self._radiohosts[name].management_ip,
                interfaces=self._radiohosts[name].interfaces,
                workload_interface=,
                workload_ip=,
            ) for radio_host_name in configs.keys()
        })                
        '''
