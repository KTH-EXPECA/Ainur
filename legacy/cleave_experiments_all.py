        ...
        "elrond": WorkloadHost(
            ...
            interfaces={
                "enp4s0": EthernetInterface(
                    ...
                    switch_connection=SwitchConnection(name="glorfindel", port=2),
                )
            },
        ),
        "workload-client-00": WorkloadHost(
            ...
            interfaces={
                "eth0": EthernetInterface(
                    ...
                    switch_connection=SwitchConnection(name="glorfindel", port=25),
                ),
                "wlan1": WiFiInterface(
                    name="wlan1",
        ...
        "workload-client-01": WorkloadHost(
            ...
            interfaces={
                "eth0": EthernetInterface(
                    ...
                    switch_connection=SwitchConnection(name="glorfindel", port=26),
                ),
                "wlan1": WiFiInterface(
                    name="wlan1",
        ...
        "workload-client-02": WorkloadHost(
            ...
            interfaces={
                "eth0": EthernetInterface(
                    ...
                    switch_connection=SwitchConnection(name="glorfindel", port=27),
                ),
                "wlan1": WiFiInterface(
                    name="wlan1",
        ...
        "workload-client-03": WorkloadHost(
            ...
            interfaces={
                "eth0": EthernetInterface(
                    ...
                    switch_connection=SwitchConnection(name="glorfindel", port=28),
                ),
                "wlan1": WiFiInterface(
                    name="wlan1",
...
class ExperimentConfig(ServiceConfig):
    ...
    service_dict: Dict[str, Any] = field(init=False, default_factory=dict, repr=False)
    ...
    def __post_init__(self):
        ...
        add_consts = "\n      ".join([f'- "{c.strip()}"' for c in self.add_constraints])
...
if __name__ == "__main__":
    ...
    load_clients = [f"workload-client-{i:02d}" for i in (6, 7, 8, 9)]
    ...
    # pull images
    docker_hosts = [
        str(host.management_ip.ip) for _, host in inventory["hosts"].items()
    ]
    ...
    with ExitStack() as stack:
        phy_layer = stack.enter_context(
            PhysicalLayer(
                inventory, workload_network_desc, ansible_ctx, ansible_quiet=True
            )
        )
        ...
        logger.warning(
            "Sleeping for 30s to allow for all devices to " "connect to network..."
        )
        time.sleep(30)
        ...
        swarm = stack.enter_context(
            DockerSwarm(
                network=workload_net, managers=dict(managers), workers=dict(workers)
            )
        )
        ...
        for exp_config in exp_configs:
            ...
            workload: WorkloadSpecification = WorkloadSpecification.from_dict(base_def)
