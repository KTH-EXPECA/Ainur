...
from ainur.hosts import AinurCloudHostConfig, EthernetCfg, LocalAinurHost, WireSpec, IPRoute
    ...
    with ExitStack() as stack:
        ...
        if phy_is_lte:
            ...
            switch_ports = [
                ...
                LTE_SERVER.ethernets["enp4s0"].wire_spec.switch_port,
                LTE_VPN_GW.ethernets["eth0"].wire_spec.switch_port,
            ]
            phy_layer._switch.make_vlan(switch_ports, name="lte_bs")
            
            # client/UE side
            switch_ports = [
                LTE_UE.ethernets["enp4s0"].wire_spec.switch_port,
                LTE_CLIENT.ethernets["eth0"].wire_spec.switch_port,
            ]
            phy_layer._switch.make_vlan(switch_ports, name="lte_ue")
        ...
        if phy_is_lte:
            ...
            input("Wait a few seconds, then press any key to start lte ue...\n")
        ...
        ).deploy_workers(hosts={LTE_CLIENT: dict(role="client", location="local")})
